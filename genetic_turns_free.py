...
class world:
    ...
    def run(self, num_generations=5, survival_rate=.35, mutation_rate=.6):
        ...
        self.population.sort(key = lambda x: x.fitness)
        self.print_population()
        self.saved_melody = self.population[0].get_pattern()
        # self.play_melodies()
...
def clean_max_input(address: str, *args: List[Any]):
    ...
    # print(ms / 1000)
    note_length = second2tick((ms/1000), tpb, bpm2tempo(bpm))
    # target_patt.append((ms, int(pitch)))
    target_patt.append((int(note_length), int(pitch)))
    tempo_list.append(user_tempo)
    # print(target_patt)
...
def listen2Max(ip,port,path, serve_time):
    ...
    max_disp = dispatcher.Dispatcher()
    # server to listen
    server = osc_server.ThreadingOSCUDPServer((ip,port), max_disp)
    print("Serving on {}".format(server.server_address))
    max_disp.map(path, clean_max_input)
    print("listening")
    endTime = datetime.datetime.now() + datetime.timedelta(seconds=serve_time)
    while(datetime.datetime.now() <= endTime):
        server.handle_request()
    max_disp.unmap(path, clean_max_input)
    # get user's tempo
    ...
    #     ms = target_patt[i][0]ç
    #     target_patt[i] = (second2tick((ms/1000), tpb, tempo_avg), target_patt[i][1])
...
# for i in range(len(rhythm_options)):
#     rhythm_options[i] = tick2second(rhythm_options[i], tpb, bpm)
